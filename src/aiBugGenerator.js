...
async function generateWithRetry(fn, retries = 3) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      console.log(`⚠️ Gemini busy... retry ${i + 1}/${retries}`);

      if (i === retries - 1) throw error;

      // espera 2s, luego 4s, luego 6s
...
async function generateBugReport(evidenceBatch, userInput) {
  const template = fs.readFileSync(
    path.join(__dirname, "../prompts/bugAgent.md"),
    "utf-8",
  );
  ...
  const promptText = template
    .replace(
      "{{evidenceType}}",
      evidenceBatch.media.length > 0 ? "Multimedia" : "Logs",
    )
